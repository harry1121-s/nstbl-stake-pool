...
contract HandlerStaker is HandlerBase {
    ...
    function stake(uint256 amount_) public {
        ...
        (stakerInfoNew.amount, stakerInfoNew.poolDebt, stakerInfoNew.epochId, stakerInfoNew.lpTokens, stakerInfoNew.stakeTimeStamp) =
            stakePool.getStakerInfo(address(this), trancheId);

        // Post-condition
        if(stakerInfo.amount == 0) { // First stake
            ...
            if(loanManager.getAwaitingRedemptionStatus(USDC)) {
                assertEq(stakerInfoNew.amount, stakerInfo.amount + amount_ - unstakeFee, "2:Should have staked the correct amount");
                ...
                assertEq(stakerInfoNew.lpTokens, stakerInfo.lpTokens + amount_, "2:Should have set the lpTokens correctly");
            }
        }
    ...
    function unstake(uint256 amount_) public { 
        ...
        if(!depeg) {
            ...
            if(block.timestamp - stakerInfo.stakeTimeStamp > trancheStakeTimePeriod[trancheId] + 1) {
                assertEq(stakerInfoNew.amount, tokensAvailable - unstakeFee, "1:Should have restaked the correct amount");
