...
contract HandlerStaker is HandlerBase {
    ...
    function stake(uint256 amount_) public {
        ...
        (stakerInfoNew.amount, stakerInfoNew.poolDebt, stakerInfoNew.epochId, stakerInfoNew.lpTokens) =
            stakePool.getStakerInfo(address(this), trancheId);
        ...
        if(stakerInfo.amount == 0) { // First stake
            ...
            if(loanManager.getAwaitingRedemptionStatus(USDC)) {
                // assertEq(stakerInfoNew.amount, stakerInfo.amount + amount_ - _getUnstakeFee(trancheId, _), "2:Should have staked the correct amount");
